package main

import "sort"

import "fmt"

func threeSum(nums []int) [][]int {
	sum := [][]int{}
	sort.Ints(nums)
	if len(nums) == 0 || nums[0] > 0 {
		return sum
	}
	intMap := make(map[int]struct{})
	for i := 0; i<len(nums); i++ {
		if _, status := intMap[nums[i]]; status {
			continue
		}
		if nums[i] == -14 {
			fmt.Println("")
		}
		intMap[nums[i]] = struct{}{}
		if nums[i] > 0 || (i>0 && nums[i] == nums[i-1]) {
			continue
		}
		mid := -nums[i]
		left := i + 1
		right := len(nums) - 1
		for {
			if left >= right {
				break
			}
			if nums[left] + nums[right] == mid {
				sum = append(sum,[]int{nums[i], nums[left], nums[right]})
				nowLeft := left
				nowRight := right
				for {
					left = left + 1
					right = right - 1
					if left >= right {
						break
					}
					if !(nums[left] == nums[nowLeft] && nums[right] == nums[nowRight]) {
						break
					}
				}
			} else if nums[left] + nums[right] > mid {
				right = right - 1
			} else {
				left = left + 1
			}
		}
	}
	return sum
}

func main() {
	fmt.Println(threeSum([]int{34,55,79,28,46,33,2,48,31,-3,84,71,52,-3,93,15,21,-43,57,-6,86,56,94,74,83,-14,28,-66,46,-49,62,-11,43,65,77,12,47,61,26,1,13,29,55,-82,76,26,15,-29,36,-29,10,-70,69,17,49}))
}